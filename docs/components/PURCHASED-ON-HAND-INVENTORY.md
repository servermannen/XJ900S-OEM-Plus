...
| [COMP-0010: B5Y-marked module candidate](COMP-0010-owner-observed-b5y-marked-module-candidate.md) | Unknown; seller item/stock ID `000001631688` is not a Yamaha part number | Baboon marketplace provenance; seller label `Snelheids Sensor` is seller terminology only and does not establish Yamaha function | Not recorded | Owner-observed on-hand module with visible label `B5Y0`, visible marking/serial `B5Y000M33S205A`, four mounting holes with metal bushings, integrated 4-position connector with blue terminal inserts, and associated connector/harness wire colours observed as red/white, blue/black, blue/white, black/white from the wire-entry side in the project-shown orientation; exact Yamaha function, part number, source model/application, connector identity, terminal family, cavity numbering, pinout, protocol, project role, safety suitability, and compatibility Unverified | Acquired / on hand candidate | COMP-0010 owner-observed physical evidence, project-reported on-hand context, and Baboon marketplace provenance only | Unverified; not accepted; technical review required |
